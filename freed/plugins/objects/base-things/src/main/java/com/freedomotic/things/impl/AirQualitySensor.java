...
public class AirQualitySensor
    ...
    private RangedIntBehaviorLogic airQuality;
    private static final String BEHAVIOR_TEMPERATURE = "airQuality";

    @Override
    public void init() {
//linking this property with the behavior defined in the XML
    	airQuality = new RangedIntBehaviorLogic((RangedIntBehavior) getPojo().getBehavior(BEHAVIOR_TEMPERATURE));
    	airQuality.addListener(new RangedIntBehaviorLogic.Listener() {
            @Override
            public void onLowerBoundValue(Config params, boolean fireCommand) {
            	if (params.getProperty("value.original").equals(params.getProperty("value"))) {
//ok here, just trying to set minimum
            		onRangeValue(airQuality.getMin(), params, fireCommand);
            	} else {
...
//ok here, just trying to set maximum
            		onRangeValue(airQuality.getMax(), params, fireCommand);
            	} else {
//there is an hardware read error
            	}
            }
            ...
            public void onRangeValue(int rangeValue, Config params, boolean fireCommand) {
                if (fireCommand) {
                	executeSetAirQuality(rangeValue, params);
                } else {
...
//register this behavior to the superclass to make it visible to it
        registerBehavior(airQuality);
        super.init();
    }

    public void executeSetAirQuality(int rangeValue, Config params) {
        boolean executed = executeCommand("set airQuality", params);
        if (executed) {
        	airQuality.setValue(rangeValue);
    ...
    private void setTemperature(int value) {
        LOG.config("Setting behavior 'airQuality' of object '" + getPojo().getName() + "' to "
                + value);

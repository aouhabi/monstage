...
public Room getRoom(String uid) {
    	...
    	while ( (r==null) && i<= getZones().size() ) {
    		 if ((getZones().get(i) instanceof Room) && (getZones().get(i).getPojo().getUuid().equals(uid)) ) {
    			 System.err.println("j ss la");
                 return (Room) getZones().get(i) ;
             }
